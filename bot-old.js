...
const ARBEJDSMAPPE = '/home/ai/escort5-auto';
...
bot.onText(/\/status/, (msg) => {
  bot.sendMessage(msg.chat.id, `✅ Lenovo server kører fint!\n🌐 Site: ${SITE_NAVN}`);
});
...
// ── Alle beskeder (session + Claude AI) ──────────────────
bot.on('message', async (msg) => {
  ...
  if (session && session.type === 'artikel') {
    ...
    if (session.step === 'by') {
      ...
      bot.sendMessage(chatId, `⏳ Genererer ${typeNavn}:\n• By: *${by}*\n• Emne: *${emne}*${extra ? `\n• Extra: ${extra}` : ''}`, { parse_mode: 'Markdown' });
      ...
      exec(
        ...
        (error) => {
          if (error) {
            ...
            bot.sendMessage(chatId, `✅ ${typeNavn.charAt(0).toUpperCase() + typeNavn.slice(1)} om *${by}* er publiceret til ${SITE_NAVN}!`, { parse_mode: 'Markdown' });
          }
        }
      );
    }
  ...
  if (session && session.type === 'billede') {
    ...
    if (session.step === 'url') {
      ...
      // Fjern Telegram markdown-formatering fra URL
session.url = tekst.replace(/^_+|_+$/g, '').replace(/__/g, '').trim();
session.step = 'mappe';
  ...
  if (session && session.type === 'sogeord') {
    ...
    if (session.step === 'mode') {
      if (tekst === '1') {
        ...
        exec(`cd ${ARBEJDSMAPPE} && node sogeord.js --antal 10`, { timeout: 60000 }, (error, stdout) => {
          if (error) {
            ...
            bot.sendMessage(chatId, `📊 *Artikelmuligheder:*\n\`\`\`\n${stdout.slice(0, 3000)}\`\`\``, { parse_mode: 'Markdown' });
          }
        });
        ...
        exec(`cd ${ARBEJDSMAPPE} && node sogeord.js --dage 180 --antal 10`, { timeout: 60000 }, (error, stdout) => {
          if (error) {
            ...
            bot.sendMessage(chatId, `🍋 *Lavt hængende frugter:*\n\`\`\`\n${stdout.slice(0, 3000)}\`\`\``, { parse_mode: 'Markdown' });
          }
        });

      } else if (tekst === '3') {
        session.step = 'antal';
        bot.sendMessage(chatId, `🔧 Hvor mange søgeord vil du se?\n_(Standard: 10)_`, { parse_mode: 'Markdown' });
      ...
      session.step = 'dage';
      bot.sendMessage(chatId, `📅 Hvor mange dage tilbage?\n_(Standard: 90)_`, { parse_mode: 'Markdown' });
      ...
      exec(
        `cd ${ARBEJDSMAPPE} && node sogeord.js --antal ${antal} --dage ${dage}`,
        { timeout: 60000 },
        (error, stdout) => {
          if (error) {
            bot.sendMessage(chatId, `❌ Fejl: ${error.message}`);
          } else {
            bot.sendMessage(chatId, `🔍 *Søgeord:*\n\`\`\`\n${stdout.slice(0, 3000)}\`\`\``, { parse_mode: 'Markdown' });
          }
        }
      );
    }
  ...
  try {
    const response = await claude.messages.create({
      ...
      system: `Du er en hjælpsom dansk AI assistent der hjælper Søren med at administrere ${SITE_NAVN}.

Du kan hjælpe med:
- /artikel - generer blog/artikel om en by
- /billede - hent billeder fra URL
- /sogeord - find søgeord fra Search Console
- /manglende - find byer uden artikler
